...
:installdeps
	...
	call :download wintun.zip https://www.wintun.net/builds/wintun-0.14.1.zip 07c256185d6ee3652e09fa55c0b90d588c35af64f6bba94319c671f2baf886c4 || goto :error
...
:sign
	...
	echo [+] Signing
	signtool sign %SigningProvider% /fd sha256 /tr "%TimestampServer%" /td sha256 /d WireGuard-GM x86\wireguard.exe amd64\wireguard.exe arm64\wireguard.exe || goto :error
...
:build_plat
	...
	go build -ldflags="-H windowsgui -s -w" -trimpath -buildvcs=false -v -o "%~1\wireguard.exe" || exit /b 1
	echo [+] Copying wintun.dll %1
